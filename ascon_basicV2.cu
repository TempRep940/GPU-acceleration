...
//Use the same data as GPU implementation
__host__ void CPU_implementation(char * plaintext_h,char * nonce, char * key,char * ad_h, char * ciphertext_h,char * decryptedtext_h){
    //This function do the CPU implementation of Ascon encryption and decryption
    char* plaintext,* ciphertext, * decryptedtext,* ad;
    
    plaintext=plaintext_h;
    ad=ad_h;
    ...
    //verify correctness according to the GPU result    //verify correctness according to the GPU result
    int error_count=0;
    for (int i = 0; i < (LENGTH+16)*SIZE; i++)
    {
        if(ciphertext[i]!=ciphertext_h[i]){
            error_count++;
        }
    }
    printf("Encryption error count: %d\n",error_count);
    ...
    //verify correctness according to the GPU result
    error_count=0;
    for (int i = 0; i < LENGTH*SIZE; i++)
    {
        if(decryptedtext[i]!=decryptedtext_h[i]){
            error_count++;
        }
    }
    printf("Decryption error count: %d\n",error_count);
...
int main(int argc, char const *argv[])
{
    ...
    //Try to use "key" and "nonce" as parameters here, you will find interesting output
    //Why does it happen? Recall the idea of ASCON-AEAD
    //=====================CPU Implementation=====================//
    CPU_implementation(plaintext_h,nonce_h, key_h,ad_h, ciphertext_h,decryptedtext_h);
    //=====================GPU Implementation=====================//
