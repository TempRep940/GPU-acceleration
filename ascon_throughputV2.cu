/*__shared__ char plaintext_s[LENGTH*BLOCK_SIZE];
    __shared__ char ad_s[LENGTH*BLOCK_SIZE];
    

    //move ad and plaintext from global memory to shared memory
    for(int i=0;i<LENGTH;i++){
        plaintext_s[threadIdx.x+i*BLOCK_SIZE]=plaintext_d[blockIdx.x * LENGTH *BLOCK_SIZE+threadIdx.x+i*BLOCK_SIZE];
        ad_s[threadIdx.x+i*BLOCK_SIZE]=ad_d[blockIdx.x * LENGTH *BLOCK_SIZE+threadIdx.x+i*BLOCK_SIZE];
    }
    __syncthreads();*/
...
__global__ void ascon_encrypt_device(char* plaintext_d, char* ad_d, char * ciphertext_d){
    ...
    */
    ...
    char* plaintext=LENGTH*threadIdx.x + plaintext_s;
    char* ad=LENGTH*threadIdx.x + ad_s;
...
__global__ void ascon_decrypt_device(char* ciphertext_d, char* ad_d, char * decryptedtext_d){
    ...
    if (global_tid>=SIZE) return;
    */
    __shared__ char ciphertext_s[(LENGTH+16)*BLOCK_SIZE];
    __shared__ char ad_s[LENGTH*BLOCK_SIZE];
    ...
    for(int i=0;i<LENGTH;i++){
        ad_s[threadIdx.x+i*BLOCK_SIZE]=ad_d[blockIdx.x * LENGTH *BLOCK_SIZE+threadIdx.x+i*BLOCK_SIZE];
        //ciphertext_s[threadIdx.x+i*BLOCK_SIZE]=ciphertext_d[blockIdx.x * (LENGTH+16) *BLOCK_SIZE+threadIdx.x+i*BLOCK_SIZE];
    }
    __syncthreads();
    ...
    char* ciphertext=(LENGTH+16)*threadIdx.x + ciphertext_s;
    char* ad=LENGTH*threadIdx.x + ad_s;
...
int main(int argc, char const *argv[])
{
    ...
    //Set kernel as Shared-Memory preferred
    //No big difference comparing to default setting
    cudaFuncSetCacheConfig(ascon_encrypt_device, cudaFuncCachePreferShared);
    cudaFuncSetCacheConfig(ascon_decrypt_device, cudaFuncCachePreferShared);
