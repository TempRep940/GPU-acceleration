//ADDED MANUALLY
#define IMPORTANT_FUNCTION __host__ __device__

//BELOW IS THE ORIGINAL FILE CONTENT
...
IMPORTANT_FUNCTION static inline uint64_t LOADBYTES(const uint8_t* bytes, int n) {
  uint64_t x = 0;
...
IMPORTANT_FUNCTION static inline void STOREBYTES(uint8_t* bytes, uint64_t x, int n) {
  for (int i = 0; i < n; ++i) bytes[i] = GETBYTE(x, i);
}

/* clear bytes in 64-bit Ascon word */
IMPORTANT_FUNCTION static inline uint64_t CLEARBYTES(uint64_t x, int n) {
